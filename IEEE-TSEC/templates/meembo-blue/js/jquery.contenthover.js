		...
		init : function(options) {
			...
			return this.each(function(){
				var $this = $(this),
					...
					overlay_w = settings.overlay_width ? settings.overlay_width : 1000,
					overlay_h = settings.overlay_height ? settings.overlay_height : 300,
					$data = $this.next(settings.data_selector);
					
				if ($data.length) {
					
					$data.hide();
                  var $ch_wrapper = $('<div>').addClass('ch_element').addClass(settings.wrapper_class).css({ 'width':'1000', 'height':'300', 'position':'absolute', 'overflow':'hidden' ,'left':'175px'}).insertAfter($this);
